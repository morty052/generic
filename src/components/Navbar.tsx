...
function Navbar({}: Props) {
  ...
  <nav className="relative max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
    ...
    <NavLink to={'/'} className="flex-none  cursor-pointer text-xl font-black text-white first-letter:text-3xl"  aria-label="Brand">Chrome<span className="text-3xl">A</span>xis</NavLink>
    ...
    <div id="navbar-collapse-with-animation" className="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow sm:block">
      <div className="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:pl-7">
        <NavLink to={'/'} className="font-medium text-white sm:py-6"  aria-current="page">Home</NavLink>
        <NavLink to={'/pricing'} className="font-medium text-white/[.8] hover:text-white sm:py-6" >Pricing</NavLink>
        <NavLink to={"/careers"} className="font-medium text-white/[.8] hover:text-white sm:py-6" >Careers</NavLink>
        <NavLink to={"/aboutus"} className="font-medium text-white/[.8] hover:text-white sm:py-6" >About us</NavLink>
        ...
        {/* <div className="hs-dropdown [--strategy:static] sm:[--strategy:fixed] [--adaptive:none] sm:[--trigger:hover] sm:py-4">
          ...
          <div className="hs-dropdown-menu transition-[opacity,margin] duration-[0.1ms] sm:duration-[150ms] hs-dropdown-open:opacity-100 opacity-0 sm:w-48 hidden z-10 bg-white sm:shadow-md rounded-lg p-2     before:absolute top-full sm:border before:-top-5 before:left-0 before:w-full before:h-5">
            ...
            <div className="hs-dropdown relative [--strategy:static] sm:[--strategy:absolute] [--adaptive:none] sm:[--trigger:hover]">
              <button type="button" className="w-full flex justify-between  items-center text-sm text-gray-800 rounded-md py-2 px-3 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500   ">
                Sub Menu
        ...
        <NavLink to={"/login"} className="flex items-center gap-x-2 font-medium text-white/[.8] hover:text-white sm:border-l sm:border-white/[.3] sm:my-6 sm:pl-6" >
